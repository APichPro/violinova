...
import { parseJSON } from '@/utils/sheet-music';
...
const SheetMusic: React.FunctionComponent<Props> = ({
  ...
  const tune = React.useRef<any>();
  const json = React.useRef<any>();
  const noteList = React.useRef<any>();
  ...
  React.useEffect(() => {
    if (notation && abcjs?.current && tune?.current && json?.current) {
      timer.current = new abcjs.current.TimingCallbacks(tune.current[0], {
        ...
        beatCallback: (beatNumber: any, totalBeats: any, totalTime: any) => {
          if (typeof onBeat === 'function') {
            onBeat(beatNumber, totalBeats, totalTime);
          }
        },
        lineEndCallback: (info: any) => {
        ...
        eventCallback: (event: any) => {
          if (typeof onEvent === 'function') {
            if (event === null) {
              ...
              const allNotes = event.startCharArray.map(
                (_: any, index: any) => {
      ...
      return () => {
        if (timer.current) {
          // Clear the timer when the component is unmounted or the effect is re-run
          timer.current.stop();
        }
      };
    }
